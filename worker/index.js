...
const Pages静态页面 = 'https://ljx1996109.github.io/cloudflare/pages';
///////////////////////////////////////////////////////全局常量和工具函数///////////////////////////////////////////////
...
export default {
	async scheduled(event, env, ctx) {
		if (env.KV) {
			const config_JSON = await 读取config_JSON(env, 'localhost', null, null);
			ctx.waitUntil(自动优选最佳IP(env, config_JSON, null));
		}
	},
		...
		if (访问路径 === 'version') {// 版本信息接口
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					const authCookie = cookies.split(';').find(c => c.trim().startsWith('auth2='))?.split('=')[1];
					...
					if (request.method === 'POST') {
						...
						if (输入密码 === (typeof 管理员密码 === 'string' ? 管理员密码.replace(/[\r\n]/g, '') : 管理员密码)) {
							...
							响应.headers.set('Set-Cookie', `auth2=${await MD5MD5(UA + 加密秘钥 + 管理员密码)}; Path=/; Max-Age=86400; HttpOnly; SameSite=Lax`);
							return 响应;
						}
					}
					...
					if (访问路径 === 'admin/init') {// 重置配置为默认值
						...
						if (访问路径 === 'admin/config.json') { // 保存config.json配置
							...
							try {
								...
								return new Response(JSON.stringify({ success: true, message: '自动优选最佳IP已在后台触发执行' }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
							} catch (error) {
								return new Response(JSON.stringify({ error: '触发失败: ' + error.message }), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
							}
					...
					响应.headers.set('Set-Cookie', 'auth2=; Path=/; Max-Age=0; HttpOnly');
					return 响应;
						...
						if (订阅类型 === 'mixed') {
							...
							订阅内容 = 其他节点LINK + 完整优选IP.map(原始地址 => {
								...
								if (协议类型 === 'ss' && !作为优选订阅生成器) {
									...
									const isNoTlsPort = ['80', '8080', '8880', '2052', '2082', '2086', '2095'].includes(String(节点端口));
									const securityParam = isNoTlsPort ? '' : 'security=tls&';
									const sniParam = isNoTlsPort ? '' : `&fp=${config_JSON.Fingerprint}&sni=example.com`;
									return `${协议类型}://00000000-0000-4000-8000-000000000000@${节点地址}:${节点端口}?${securityParam}type=${传输协议 + ECHLINK参数}&${域名字段名}=example.com${sniParam}&${路径字段名}=${encodeURIComponent(传输路径参数值) + TLS分片参数}&encryption=none#${encodeURIComponent(节点备注)}`;
								}
...
async function 生成随机IP(request, count = 16, 指定端口 = -1) {
	...
	const randomIPs = ipList.map((item, index) => {
		let ip = item;
		// API可能返回 "1.1.1.1#备注" 格式，我们需要去掉原有备注，套用我们的标准备注
		if (ip.includes('#')) ip = ip.split('#')[0];
		
		const 目标端口 = 指定端口 === -1 ? cfport[Math.floor(Math.random() * cfport.length)] : 指定端口;
		
		// 检查IP是否已经包含了端口（比如域名带端口或者IPv4带端口）
		let hasPort = false;
		if (ip.startsWith('[')) {
			hasPort = /\]:\d+$/.test(ip);
		} else {
